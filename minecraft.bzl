...
def _mojang_minecraft_jars_impl(
        repository_ctx,
        **kwargs):
    ...
    for dist in ("client", "server"):
        repository_ctx.download(downloads[dist]["url"], output = "minecraft_%s.jar" % dist)

mojang_minecraft_jars = repository_rule(
    implementation = _mojang_minecraft_jars_impl,
    local = True,
    attrs = {
        "version": attr.string(),
